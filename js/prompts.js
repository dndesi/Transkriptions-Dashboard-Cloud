...
function renderPromptsView() {
  ...
  <div style="max-width:800px; margin:0 auto; padding:8px 0 40px">
    ...
    ${prompts.length === 0 ? `
      <div style="text-align:center; padding:60px 24px; color:var(--muted); border:1px dashed var(--border); border-radius:14px">
        <div style="margin-bottom:12px; opacity:0.3">${icon('sparkles',36)}</div>
        <div style="font-size:0.9rem; margin-bottom:8px; font-weight:500">Noch keine eigenen Prompts</div>
        <div style="font-size:0.8rem; margin-bottom:20px">Erstelle deinen ersten Prompt und verwende ihn direkt im Analyse-Modal.</div>
        <button class="btn btn-primary" onclick="openPromptEditorModal(null)" style="gap:6px">
          ${icon('plus',14)} Ersten Prompt erstellen
        </button>
      </div>
    ` : `
