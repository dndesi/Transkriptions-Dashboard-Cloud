...
function renderArchView() {
  ...
  <div style="max-width:960px; margin:0 auto; padding:8px 0 40px">
    ...
    <div style="margin-bottom:24px; display:flex; align-items:flex-start; justify-content:space-between; flex-wrap:wrap; gap:12px">
      <div>
        ...
        <p style="font-size:0.82rem; color:var(--muted); line-height:1.6; margin:0">
          ...
          <span style="color:var(--accent); font-weight:600">Version 6.47</span> · 26 JS-Module
        </p>
      </div>
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:10px; margin-bottom:24px">
      ...
      ${flowCard('scan.js', 'Scan-Import', 'v6.28: Foto(s)/Kamera hochladen → OCR → parsePlainText() → eine Session pro Upload (mehrere Fotos = mehrseitige Notiz). session.source = "scan_import", ein Sprecher, kein Dialog. startScanImport() · v6.32: zwei OCR-Engines wählbar – _ocrImage() (Claude Vision, auch Handschrift) oder _ocrImageTesseract() (Tesseract.js, lokal, kein API-Key, keine Copyright-Verweigerung bei Buchseiten, schwächer bei Handschrift) · v6.33: _scanFiles nach file.lastModified sortiert, _renderScanFileList() zeigt Reihenfolge zur Kontrolle · v6.34: manuelles Umsortieren (_scanMoveFile), Entfernen (_scanRemoveFile), Zurücksetzen (_scanResetFiles), Doppelseiten-Split (_splitImageHalves) für aufgeschlagene Buchseiten · v6.35: Bugfix – _ocrImageTesseract() läuft jetzt auch durch _resizePhoto() (EXIF-Ausrichtung normalisiert, 2000px), vorher erzeugte Tesseract bei rotierten Fotos Wortsalat · v6.37: showTranscript() (claude.js) blendet #transkriptAufnahmeBlock und #transkriptSprecherBlock im Text-Tab komplett aus (kein Audioplayer, kein Sprecher-Tausch bei Scan-Notizen) · v6.38: Bugfix – startScanImport() baut Utterances jetzt direkt aus pageTexts (1 Foto = 1 Seite) statt über parsePlainText() zu re-splitten, das Absätze fälschlich als eigene Seiten zählte · v6.39: dritte Engine _ocrImagePaddleOCR() (Bibliothek "ppu-paddle-ocr" per CDN, komplett lokal/kein API-Key), Detection-Modell erkennt Textblöcke einzeln statt als durchgehenden Strom – robuster bei Spalten-/Label-Layouts als Tesseract · v6.40: nach erfolgreichem Test PaddleOCR Standard-Engine, _ocrImageTesseract() entfernt, Tesseract.js-CDN-Script aus index.html entfernt · v6.41: Modell-Stufe "Small" statt "Tiny", minimumConfidence 0.5→0.4 (gegen komplett verworfene Textblöcke), models/paddleocr/-Ordner mit README für geplantes Modell-Vendoring angelegt · v6.42: Modell-Dateien liegen jetzt lokal in models/paddleocr/ statt bei externer GitHub-Quelle geladen zu werden, unabhängig von deren Fortbestand · v6.43: Small/Medium im Vergleichstest schlechter als Tiny (Medium sogar am schlechtesten + 3x langsamer) – zurückgerollt auf Tiny + Standard-Schwelle 0.5, models/paddleocr/ enthält nur noch Tiny-Dateien · v6.44: _reflowOcrText() zieht buchzeilenweise erkannten Text zu Fließtext zusammen (Silbentrennung korrekt aufgelöst, nummerierte Listen bleiben eigene Zeilen), rein lokale Text-Verarbeitung ohne API · v6.45: PDF-Upload – _pdfToImageFiles() rendert jede Seite per PDF.js auf Canvas (scale 2.0 ≈ 144 dpi) als JPEG, handleScanFileSelect() ist jetzt async und erkennt PDFs automatisch, accept="image/*,application/pdf"', '#2dd4bf')}
