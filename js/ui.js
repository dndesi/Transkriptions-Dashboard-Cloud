    ...
    <div style="margin-bottom:24px; display:flex; align-items:flex-start; justify-content:space-between; flex-wrap:wrap; gap:12px">
      <div>
        ...
        <p style="font-size:0.82rem; color:var(--muted); line-height:1.6; margin:0">
          ...
          <span style="color:var(--accent); font-weight:600">Version 5.47</span>
        </p>
      </div>
...
function _refreshAnalysenSubtabs() {
  ...
  const visible = blockMap.filter(b => {
    ...
    if (!el) return false;
    // Custom-Blöcke: existieren im DOM nur wenn Inhalt da ist (renderInsights erstellt sie dynamisch)
    if (b.id.startsWith('customBlock_')) return true;
    // Built-in-Blöcke: data-has-content Marker (gesetzt durch showInsightsBlock / else-Zweige in renderInsights)
    // switchAnalysenSubtab ändert nur display, nicht diesen Marker → zuverlässig
